...
def _build_glm_kpool_indexer(
    ...
    *,
    ...
    """Build the KPool indexer missing from v0.5.11's MLA constructor."""
...
def swiglu_clamped(y: torch.Tensor, limit: float) -> torch.Tensor:
    """GLM-5.3 SwiGLU: clamp gate/up before applying SiLU."""
...
class _ClampedSiluAndMul(nn.Module):
    def __init__(self, limit: float):
        super().__init__()
        self.limit = float(limit)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return swiglu_clamped(x, self.limit)


class Glm5NextMLP(DeepseekV2MLP):
    """0.5.11 DeepSeek MLP with GLM's activation clamp restored."""

    def __init__(self, *args, swiglu_limit=None, **kwargs):
        super().__init__(*args, **kwargs)
        self.swiglu_limit = swiglu_limit
        if swiglu_limit is not None:
            self.act_fn = _ClampedSiluAndMul(swiglu_limit)


class Glm5NextMoEGate(MoEGate):
    """GLM router with the checkpoint's required FP32 accumulation/output."""

    def __init__(self, config, *args, **kwargs):
        super().__init__(config, *args, **kwargs)
        router_dtype = getattr(config, "moe_router_dtype", "float32")
        if router_dtype not in ("float32", torch.float32):
            raise ValueError(f"unsupported GLM MoE router dtype: {router_dtype}")
        self.weight = nn.Parameter(
            self.weight.to(dtype=torch.float32),
...
class Glm5NextMoE(DeepseekV2MoE):
    """Mark routed/shared experts for the plugin Ascend clamp path."""
    ...
    def __init__(
        ...
        # DeepseekV2MoE constructs ``self.topk`` before this GLM-specific gate
        # replacement.  Its TopKConfig keeps the original correction-bias
        # object by reference, so leaving it untouched would route with a
        # detached, never-loaded ``torch.empty`` tensor.  Rebind it to the
        # registered GLM gate parameter that the checkpoint loader updates.
        _rebind_glm_moe_correction_bias(self)
        limit = getattr(config, "swiglu_limit", None)
        if limit is not None:
            self.experts._sglang_fl_swiglu_limit = float(limit)
            if getattr(self, "shared_experts", None) is not None:
                self.shared_experts.swiglu_limit = float(limit)
    ...
    def forward(self, hidden_states, forward_batch):
        ...
        with dispatch_scope:
            return super().forward(hidden_states, forward_batch)


def _rebind_glm_moe_correction_bias(moe: Glm5NextMoE) -> None:
    """Keep TopK's cached bias reference attached to the loaded GLM gate."""

    moe.topk.topk_config.correction_bias = moe.gate.e_score_correction_bias


class Glm5NextVisionMLP(GlmOcrVisionMLP):
    def __init__(self, *args, swiglu_limit: float, **kwargs):
        super().__init__(*args, **kwargs)
...
class Glm5NextVisionModel(GlmOcrVisionModel):
    """GLM OCR ViT topology with checkpoint-specific clamped SwiGLU."""
    ...
    def __init__(
        ...
        projection_size = (
            getattr(vision_config, "projection_intermediate_size", None)
            or vision_config.intermediate_size
        )
        self.merger = Glm5NextVisionPatchMerger(
            d_model=vision_config.out_hidden_size,
            context_dim=projection_size,
            quant_config=quant_config,
            bias=False,
...
def _prepare_glm_kda_gates(
    ...
    *,
    ...
    """Adapt GLM gate projections to v0.5.11 KDA kernel layouts."""

    if not is_decode:
        forget_gate = forget_gate.unflatten(-1, (-1, head_dim))
...
class Glm5NextLinearAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # The Ascend recurrent KDA path consumes raw forget/beta projections
        # and performs both activations internally.  Prefill only needs to
        # expose the gate's [H,K] axes before adding its batch dimension;
        # decode keeps the flattened ABI already captured by NPUGraph.
        forget_gate, beta = _prepare_glm_kda_gates(
            forget_gate,
            beta,
...
class Glm5NextDecoderLayer(nn.Module):
    def __init__(
        ...
        if self.is_linear_attn:
            ...
            # SGLang v0.5.11 hard-codes the regular NSA Indexer above.  The
            # GLM checkpoint uses compressed KPool indexing and contains its
            # additional gate/APE weights.  Replace it before weight loading,
            # scoped to this external model instead of changing SGLang's
            # generic DeepseekV2AttentionMLA class.
            if config.index_kpool > 1:
                self.self_attn.indexer = _build_glm_kpool_indexer(
                    config,
                    hidden_size=self.hidden_size,
                    layer_id=layer_id,
                    max_position_embeddings=max_position_embeddings,
    ...
    def forward(
        ...
        if mla_cp_wrap:
            ...
            set_attn_hidden_states_local(get_attn_tp_context(), hidden_states)
        ...
        if isinstance(hidden_states, tuple):
            ...
            topk_indices = None
        clear_attn_inputs(get_attn_tp_context())
...
class Glm5NextForConditionalGeneration(nn.Module):
    ...
    def __init__(
        ...
        get_attn_tp_context().init_context(
            getattr(text_config, "q_lora_rank", None), self.use_dsa
        )
    ...
    def determine_num_fused_shared_experts(self):
        ...
        if disable_reason is not None:
            # v0.5.11 predates the load-time override helper.  The worker owns
            # its ServerArgs instance, so applying the same override directly
            # is safe and is observed by DeepseekV2MoE constructors below.
