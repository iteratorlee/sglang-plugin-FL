...
class Glm5NextTextConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # transformers 5.6 validates a fixed layer-type vocabulary.  The
        # checkpoint's vendor-specific name carries the same sparse-attention
        # meaning, while linear/full layer identity is already preserved in
        # ``linear_attn_config`` above.
        if "layer_types" in kwargs:
            kwargs["layer_types"] = [
                "sparse" if value == "deepseek_sparse_attention" else value
                for value in kwargs["layer_types"]
            ]
...
class Glm5NextVisionConfig(GlmOcrVisionConfig):
    """Preserve the GLM-5.3 vision activation-clamp parameter explicitly."""
...
class Glm5NextConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # SGLang 0.5.11's GLM vision implementation expects the two derived
        # fields normally injected by its known-model config updater.  GLM-5.3
        # is registered out of tree, so derive them here with the exact same
        # rule.  For the shipped checkpoint this is 1024/16=64 and TP16 needs
        # no padding.
        vision_num_heads = getattr(
            self.vision_config,
            "num_heads",
            getattr(self.vision_config, "num_attention_heads", None),
        )
        if vision_num_heads:
            self.vision_config.head_dim = (
                self.vision_config.hidden_size // vision_num_heads
            )
            # Config parsing happens before SGLang initializes the TP group.
            # Keep this phase pure-data; the model constructor invokes
            # SGLang's own update_vit_attn_dummy_heads_config after group init.
            self.vision_config.num_dummy_heads = 0
        ...
        # SGLang v0.5.11 classifies MLA/NSA with architecture-name allowlists
        # while external model discovery uses the first outer architecture.
        # Preserve the real GLM entry first, append its closest built-in MLA
        # marker, and give only the nested text config the built-in DSA marker.
        # This lets the unmodified ModelConfig derive MLA cache dimensions
        # before the later general-plugin hook is invoked.
        self.architectures = [
            "Glm5NextForConditionalGeneration",
            "GlmMoeDsaForCausalLM",
        ]
        self.text_config.architectures = ["GlmMoeDsaForCausalLM"]
